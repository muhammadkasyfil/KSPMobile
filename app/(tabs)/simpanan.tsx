...
export default function SimpananPage() {
  ...
  // Helper to load transactions for selected account
  const loadTransactions = async (acct: SavingsAccount) => {
    try {
      const token = await AsyncStorage.getItem('token');
      if (!token) return;
      const load = async (status: string) => {
        const res = await fetch(
          `https://kspdigital-api.up.railway.app/api/simpanan/transaksi-simpanan/${status}/?jenis_simpanan=${acct.id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        if (!res.ok) return [];
        ...
        return (json.results || []).map((it: any) => ({
          ...
          nominal: it.nominal,
          tanggal_transaksi: new Date(it.tanggal_transaksi).toLocaleDateString('id-ID', {
            day: 'numeric',
            month: 'long',
            year: 'numeric',
          }),
  ...
  // Fetch transactions when selected changes
  useEffect(() => {
  ...
  const submitWithdrawal = async () => {
    if (!withdrawalData.amount || isNaN(Number(withdrawalData.amount))) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        Alert.alert('Gagal', json.error || JSON.stringify(json));
      }
    } catch (e) {
  ...
  const resetModal = () => {
    setStep(0);
    setWithdrawalData({ method: '', time: '', amount: '', savingsType: '' });
  };
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.container}>
        ...
        <View style={styles.balanceCard}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              setWithdrawalData(v => ({ ...v, savingsType: selected.title }));
              next();
            }}
          >
              ...
              {step === 1 && (
                <View>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setWithdrawalData(v => ({ ...v, method: 'Kantor' }));
                      next();
                    }}
                  >
                  ...
                  <TouchableOpacity
                    style={styles.methodCard}
                    onPress={() => {
                      setWithdrawalData(v => ({ ...v, method: 'Rumah' }));
                      next();
                    }}
                  >
                ...
                <View>
                  ...
                  {['8.00 - 10.00', '10.00 - 12.00', '12.00 - 14.00', '14.00 - 16.00'].map(t => (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        setWithdrawalData(v => ({ ...v, time: t }));
                        next();
                      }}
                    >
                      <Text style={styles.radioText}>{t}</Text>
                      <FontAwesome5 name={withdrawalData.time === t ? 'dot-circle' : 'circle'} color="#3c9a4b" />
                    </TouchableOpacity>
                  ))}
                </View>
              )}
                ...
                <View>
                  ...
                  <View style={styles.formGroup}>
                    ...
                    <View style={[styles.input, styles.select]}>
                      <Text style={styles.selectText}>{withdrawalData.savingsType}</Text>
                    </View>
                  </View>
                  <View style={styles.formGroup}>
                    ...
                    <TextInput
                      ...
                      onChangeText={v => setWithdrawalData(vd => ({ ...vd, amount: v }))}
                    />
                    <View style={styles.infoContainer}>
                      <FontAwesome5 name="info-circle" size={12} color="#666" />
                      <Text style={styles.limitText}>Saldo tersedia: Rp {selected.amount.toLocaleString('id-ID')}</Text>
                    </View>
                  </View>
                  <TouchableOpacity style={[styles.primaryButton, { backgroundColor: '#FFD700' }]} onPress={submitWithdrawal}>
        ...
        {pending.map(tx => <TransactionItem key={tx.id} tx={tx} />)}
        <Text style={styles.sectionTitle}>Completed</Text>
        {completed.map(tx => <TransactionItem key={tx.id} tx={tx} />)}
        <Text style={styles.sectionTitle}>Cancelled</Text>
        {cancelled.map(tx => <TransactionItem key={tx.id} tx={tx} />)}
      </ScrollView>
    );
  }
  ...
  return (
    <ScrollView style={styles.scrollView} contentContainerStyle={styles.container}>
      ...
      <View style={styles.cardsGrid}>
        {accounts.map(acc => (
          <TouchableOpacity key={acc.id} style={styles.card} onPress={() => setSelected(acc)}>
            <View style={styles.iconContainer}>
              <FontAwesome5 name={acc.icon} size={24} color="white" />
            </View>
            <Text style={styles.cardTitle}>{acc.title}</Text>
            <Text style={styles.cardAmount}>Rp {acc.amount.toLocaleString('id-ID')}</Text>
          </TouchableOpacity>
        ))}
  ...
  <View style={styles.transactionItem}>
    <View
      style={[
        ...
        { backgroundColor: tx.jenis_transaksi === 'setoran' ? '#e6f4ea' : '#fce8e8' },
      ]}
    >
      <FontAwesome5
        name={tx.jenis_transaksi === 'setoran' ? 'arrow-down' : 'arrow-up'}
        size={16}
        color={tx.jenis_transaksi === 'setoran' ? '#3c9a4b' : '#d32f2f'}
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  card: {
    width: '48%', backgroundColor: 'white', borderRadius: 16, padding: 20,
    marginBottom: 16, alignItems: 'center', shadowColor: '#000',
    shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1,
    shadowRadius: 2, elevation: 2
  },
  iconContainer: { width: 60, height: 60, borderRadius: 30, backgroundColor: '#3c9a4b',
  ...
  balanceCard: {
    backgroundColor: '#FFD700', borderRadius: 16, padding: 20, marginBottom: 24,
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.1, shadowRadius: 4, elevation: 3
  },
  balanceHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 16 },
  balanceIconContainer: {
    width: 50, height: 50, borderRadius: 25, backgroundColor: '#3c9a4b',
  ...
  transactionItem: {
    ...
    borderRadius: 12, padding: 16, marginBottom: 12, shadowColor: '#000',
  ...
  noTransactionsText: {
    fontSize: 14, color: '#777', fontStyle: 'italic', backgroundColor: 'white',
  ...
  methodCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff8dc',
    padding: 14, borderRadius: 10, marginBottom: 12, elevation: 2
  },
  ...
  input: { borderWidth: 1, borderColor: '#ccc', borderRadius: 8, paddingHorizontal: 12,
  ...
  primaryButton: { backgroundColor: '#3c9a4b', padding: 14, borderRadius: 8,
